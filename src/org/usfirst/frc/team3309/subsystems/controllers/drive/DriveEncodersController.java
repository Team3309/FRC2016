...
public class DriveEncodersController extends Controller {
	...
	private PIDPositionController leftController = new PIDPositionController(0,
			0, 0);
	private PIDPositionController rightController = new PIDPositionController(
			0, 0, 0);
	private PIDPositionController angController = new PIDPositionController(0,
			0, 0);
	...
	public OutputSignal getOutputSignal(InputState inputState) {
		...
		OutputSignal leftOutput = leftController
				.getOutputSignal(inputForLeftVel);
		OutputSignal rightOutput = rightController
				.getOutputSignal(inputForRightVel);
	...
	public boolean isCompleted() {
		return leftController.isCompleted() && rightController.isCompleted()
				&& angController.isCompleted();
	}
}
