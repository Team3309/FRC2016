...
public class DriveAngleVelocityController extends Controller {
	private FeedForwardWithPIDController leftSideController = new FeedForwardWithPIDController(Drive.getInstance(),
			false, .006, 0, .003, .001, 0);
	private FeedForwardWithPIDController rightSideController = new FeedForwardWithPIDController(Drive.getInstance(),
			false, .006, 0, .003, .001, 0);
	public PIDPositionController turningController = new PIDPositionController(Drive.getInstance(), false, 3, 0,
			16.015);
	...
	public DriveAngleVelocityController(ControlledSubsystem system, double aimAngle) {
		super(system);
	...
	public void update(InputState inputState) {
		...
		turningController.update(state);
		OutputSignal outputOfTurningController = turningController.getOutputSignal(); // output
		...
		double leftOutput = -leftSideController.getOutputSignal().getMotor();
		double rightOutput = rightSideController.getOutputSignal().getMotor();
		try {
			System.out.println(loopTimer.get() + ", " + inputState.getLeftPos() + ", " + inputState.getRightPos() + ", "
					+ leftOutput + ", " + rightOutput + ", " + this.goalAngle + ", " + inputState.getAngularPos());
			;
		} catch (SensorDoesNotReturnException e) {
			e.printStackTrace();
		}
		toBeReturnedSignal.setLeftRightMotor(leftOutput, rightOutput);
